...
class HomeController extends Controller
{
	...
	public function downloads(){
		...
		$cat_id=Download_service::search()->get();
		$catalog=Download_service::where('type','catalog')->get();
		$pricelist=Download_service::where('type','pricelist')->get();
		$manuals=Download_service::where('type','manuals')->get();
		$software=Download_service::where('type','software')->get();
	...
	public function view_doc($slug,Request $req){
		$download= Download_service::where('slug',$slug)->first();
		$download2= Download_service::where('slug',$slug)->first();
		// dd($slug);	
			if($download)
			{
				$download= Download_service::where('id',$download->id);
				$download = $download->paginate(10);
			
				return view('home.download-doc',['download'=>$download,'download2' => $download2]);
			}
			else
			{
				return view('errors.404');
